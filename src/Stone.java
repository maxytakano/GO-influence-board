...
public abstract class Stone
{
	...
	{
		// If the last bit is a one, then the number is odd, so account for
		// that by altering the width to an even number.
		//
		if( (width & 1) == 1 )
			width--;
	...
	/**
	 * @return true - The stones are the same colour.
	 */
	public boolean equals( Stone stone )
	{
		return (isWhite() && stone.isWhite()) || (!isWhite() && !stone.isWhite());
	}
